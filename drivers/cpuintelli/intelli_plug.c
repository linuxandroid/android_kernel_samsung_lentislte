...
static unsigned int calculate_thread_stats(void)
{
	...
	dprintk("%s: AVG_NR_RUNNING:%d\n", INTELLI_PLUG, avg_nr_run);
...
static void __ref cpu_up_down_work(struct work_struct *work)
{
	...
	dprintk("%s: online_cpus:%d\n", INTELLI_PLUG, online_cpus);
...
static void intelli_plug_work_fn(struct work_struct *work)
{
	if (hotplug_suspended) {
		dprintk("%s: is suspended!\n", INTELLI_PLUG);
		return;
	}

	target_cpus = calculate_thread_stats();
	dprintk("%s: target_cpus:%d\n", INTELLI_PLUG, target_cpus);
